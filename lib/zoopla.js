...
module.exports.Zoopla = class Zoopla {
  ...
  async fetchProperties(pageSize = 100, maxCallsPerSession = 100) {
    const httpParams = {
      area: 'london',
      radius: 0.1,
      minimum_beds: 2,
    ...
    this.file = fs.createWriteStream(`./reports/${Date.now()}.json`, {
      flags: 'w',
      encoding: 'utf-8',
    });
    ...
    log('END');
    this.writeData(this.storedData);
  }
  ...
  writeData(data) {
    this.file.write(JSON.stringify(data, null, 2));
    this.file.end();
  }

  fetchReport(fileName) {
    var json = require(`../reports/${fileName}.json`);
